...
export default class Demo extends Phaser.Scene {
  ...
  create() {
    ...
    const question = this.add.text(window.outerWidth /2, 100, this.questions[this.currentQuestionIndex], { fontSize: '10em', color: 'green'});
    ...
    var singleButtonWidth = window.outerWidth / keys[0].length
    const singleButtonHeight = 40
    keys.forEach( (_, row) => {
      _.forEach((__, col) => {
        let x = col * singleButtonWidth + 10
        let y = (row * singleButtonHeight) + window.outerHeight - 120
        if(__ === '.') return
